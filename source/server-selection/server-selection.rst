...
is -1 or null, depending on the language. The default ``hedge`` is empty.
...
The read preference ``hedge`` parameter is an object that configures how the
...
2.  If the ``mode`` field is "primary", the ``tags``, ``maxStalenessSeconds``,
    ...
    The ``hedge`` field MUST be either absent or be an object.
...
2020-02-05: Specify read preferences with support for server hedged reads
